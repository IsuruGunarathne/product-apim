...
/**
 * Integration tests for SSRF protection (outbound request security).
 */
public class SSRFProtectionTestCase extends APIMIntegrationBaseTest {

    private static final Log log = LogFactory.getLog(SSRFProtectionTestCase.class);
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist disabled — endpoint validation is not blocked by allowlist")
    public void testAllowlistDisabled_EndpointValidationNotBlocked() throws Exception {
        // Default: EnableHostAllowlist=false. The SSRF allowlist must not trigger.
        ...
        if (dto.getError() != null) {
            Assert.assertFalse(dto.getError().contains("not trusted"),
                    "URL should not be SSRF-blocked when allowlist is disabled, error: " + dto.getError());
        }
    }

    @Test(groups = {"wso2.am"},
            description = "SSRF: genuine DNS failure should reported as a connectivity error.",
            dependsOnMethods = "testAllowlistDisabled_EndpointValidationNotBlocked")
    public void testNonExistentHost_DNSFailureNotTreatedAsSSRFBlock() throws Exception {
        ...
        Assert.assertFalse(dto.getError().contains("not trusted"),
                "DNS/connection failure must not be mis-classified as an SSRF block, error: " + dto.getError());
    }

    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — non-matching URL blocked on endpoint validation",
            dependsOnMethods = "testNonExistentHost_DNSFailureNotTreatedAsSSRFBlock")
    public void testAllowlistEnabled_NonMatchingURLBlockedOnEndpointValidation() throws Exception {
        try {
            ...
            Assert.assertNotNull(dto.getError(), "Expected an error for SSRF-blocked URL");
            Assert.assertTrue(
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — matching URL passes endpoint validation",
    ...
    public void testAllowlistEnabled_MatchingURLPassesEndpointValidation() throws Exception {
        try {
            ...
            if (dto.getError() != null) {
                Assert.assertFalse(dto.getError().contains("not trusted"),
                        "URL matching the allowlist should not be SSRF-blocked, error: " + dto.getError());
            }
        } finally {
            restoreOriginalTenantConfig();
        }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — KM create with non-matching URL returns 400",
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — KM JWKS cert URL blocked returns 400",
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — KM update with non-matching URL returns 400",
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — GraphQL schema validation by non-matching URL returns isValid=false",
    ...
    public void testAllowlistEnabled_GraphQLValidationByBlockedURLFails() throws Exception {
        try {
            ...
            Assert.assertFalse(Boolean.TRUE.equals(response.isIsValid()),
                    "GraphQL validation should fail (isValid=false) for SSRF-blocked URL");
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — WSDL import by non-matching URL returns 400",
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — OAS definition import from non-matching URL returns 400",
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — KM discover with non-matching URL returns 400",
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allowlist enabled — MCP proxy create with non-matching URL returns 400",
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: super-tenant allowlist must not affect a new tenant whose config is at default",
    ...
    public void testPrivateNetworkBlock_MultipleAPISurfacesBlocked() throws Exception {
        ...
        serverConfigManager.applyConfiguration(new File(getAMResourceLocation()
                + File.separator + "configFiles" + File.separator + "ssrfPrivateBlock"
        ...
        try {
            ...
            try {
                restAPIAdmin.addKeyManager(buildKeyManagerDTO("SSRFLinkLocalKM",
            ...
            try {
                restAPIPublisher.importWSDLSchemaDefinition(null,
                        ...
                        "{\"name\":\"SSRFLoopbackWSDL\",\"context\":\"/ssrfloopbackwsdl\","
                                + "\"version\":\"1.0.0\",\"provider\":\"" + user.getUserName() + "\"}",
                        "SOAP");
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: deny_all mode and disabled mode each in their own server lifecycle",
    ...
    public void testAllPlatformConfigCombinations() throws Exception {
        ...
        log.info("SSRF scenario: deny_all + bpna=true");
        ServerConfigurationManager mgr1 = new ServerConfigurationManager(gatewayContextMgt);
        mgr1.applyConfiguration(
                new File(configs + "ssrfDenyAllBpnaEnabled" + File.separator + "deployment.toml"));
        try {
