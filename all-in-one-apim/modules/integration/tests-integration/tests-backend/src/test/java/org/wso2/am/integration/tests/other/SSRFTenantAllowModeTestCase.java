...
/**
 * Integration tests for SSRF protection via tenant-level allow mode (NetworkSecurityAccessControl).
 * Runs under {@link SSRFTenantAllowModeTestSuite}, which applies a platform-level "allow all" configuration
 * (no mode restriction, block_private_network_access=false), isolating tenant-level allow mode behaviour.
 */
public class SSRFTenantAllowModeTestCase extends APIMIntegrationBaseTest {

    private static final Log log = LogFactory.getLog(SSRFTenantAllowModeTestCase.class);
    ...
    private static final String TENANT_DOMAIN        = "ssrftest.com";
    private static final String TENANT_ADMIN_USERNAME = "ssrfTestAdmin";
    ...
    public void setEnvironment() throws Exception {
        ...
        APIRequest apiRequest = new APIRequest("SSRFTestAPI", "/ssrftest",
        ...
        // Create an MCP server for MCP-update and backend-update SSRF tests.
        File mcpOasFile = new File(getAMResourceLocation()
                + File.separator + "ssrf" + File.separator + "ssrf-test-openapi.yaml");
        JSONObject mcpAdditionalProps = new JSONObject();
        mcpAdditionalProps.put("name", "ssrfMcpValid");
        mcpAdditionalProps.put("version", "v1");
        mcpAdditionalProps.put("context", "/ssrfmcpvalid");
        ...
        Assert.assertNotNull(backendId,
                "Setup failed: could not retrieve backendId from MCP server — backend update SSRF tests will be skipped");
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode disabled — endpoint validation is not blocked")
    public void testAllowModeDisabled_EndpointValidationNotBlocked() throws Exception {
        ...
        if (dto.getError() != null) {
            Assert.assertFalse(dto.getError().contains("not trusted"),
                    "URL should not be SSRF-blocked when allow mode is disabled, error: " + dto.getError());
        }
    }

    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — non-matching URL blocked on endpoint validation",
    ...
    public void testAllowModeEnabled_NonMatchingURLBlockedOnEndpointValidation() throws Exception {
        try {
            ...
            Assert.assertTrue(dto.getError().contains("not trusted"),
                    "Expected SSRF block message, got: " + dto.getError());
        } finally {
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — matching URL passes endpoint validation",
    ...
    public void testAllowModeEnabled_MatchingURLPassesEndpointValidation() throws Exception {
        try {
            ...
            if (dto.getError() != null) {
                Assert.assertFalse(dto.getError().contains("not trusted"),
                        "URL matching the allow mode hosts should not be SSRF-blocked, error: " + dto.getError());
            }
        } finally {
            restoreOriginalTenantConfig();
        }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — KM create with non-matching URL returns 400",
    ...
    public void testAllowModeEnabled_KeyManagerCreateWithBlockedURLRejected() throws Exception {
        try {
            enableTenantAllowMode(new String[]{"*.allowed.example.com"});
            restAPIAdmin.addKeyManager(buildKeyManagerDTO("SSRFBlockedKM",
                    ...
                    null));
            Assert.fail("Expected ApiException for KM with SSRF-blocked URLs");
        } catch (ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for KM creation with SSRF-blocked URL");
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — KM JWKS cert URL blocked returns 400",
    ...
    public void testAllowModeEnabled_KeyManagerJWKSCertURLBlocked() throws Exception {
        try {
            ...
            restAPIAdmin.addKeyManager(buildKeyManagerDTO("SSRFBlockedJWKSKM",
                    ...
                    blockedCerts));
            Assert.fail("Expected ApiException for KM with SSRF-blocked JWKS cert URL");
        } catch (ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for KM creation with SSRF-blocked JWKS URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in KM JWKS cert response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — KM update with non-matching URL returns 400",
    ...
    public void testAllowModeEnabled_KeyManagerUpdateWithBlockedURLRejected() throws Exception {
        ...
        try {
            ...
            ApiResponse<KeyManagerDTO> createResponse = restAPIAdmin.addKeyManager(
                    buildKeyManagerDTO("SSRFUpdateTestKM",
                            "https://api.allowed.example.com/oauth2/introspect",
                            "https://api.allowed.example.com/keymanager-operations/dcr/register",
                            "https://api.allowed.example.com/oauth2/token",
                            "https://api.allowed.example.com/oauth2/revoke",
            ...
            try {
                ...
                Assert.fail("Expected ApiException for KM update with SSRF-blocked URL");
            } catch (ApiException e) {
                Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                        "Expected HTTP 400 for KM update with SSRF-blocked URL");
                Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                        "Expected SSRF block error in KM update response body, got: " + e.getResponseBody());
            }
        } finally {
            if (createdKmId != null) {
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — GraphQL schema validation by non-matching URL returns isValid=false",
    ...
    public void testAllowModeEnabled_GraphQLValidationByBlockedURLFails() throws Exception {
        try {
            ...
            Assert.assertNotNull(response.getErrorMessage(),
                    "GraphQL validation response must include an error message for SSRF-blocked URL");
            Assert.assertTrue(response.getErrorMessage().contains("not trusted"),
                    "Expected SSRF block error in GraphQL validation errorMessage, got: " + response.getErrorMessage());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — WSDL import by non-matching URL returns 400",
    ...
    public void testAllowModeEnabled_WSDLImportByBlockedURLRejected() throws Exception {
        try {
            ...
            restAPIPublisher.importWSDLSchemaDefinition(null,
                    ...
                    "{\"name\":\"SSRFWSDLBlockTest\",\"context\":\"/ssrfwsdlblock\","
                            + "\"version\":\"1.0.0\",\"provider\":\"" + user.getUserName() + "\"}",
                    "SOAP");
            Assert.fail("Expected ApiException for WSDL import from SSRF-blocked URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for WSDL import from SSRF-blocked URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — OAS definition validate from non-matching URL returns 400",
    ...
    public void testAllowModeEnabled_OASDefinitionFromBlockedURLRejected() throws Exception {
        try {
            ...
            Assert.fail("Expected ApiException for OAS validate from SSRF-blocked URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for OAS definition validate from SSRF-blocked URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in OAS validate response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — KM discover with non-matching URL returns 400",
    ...
    public void testAllowModeEnabled_KeyManagerDiscoverWithBlockedURLRejected() throws Exception {
        try {
            ...
            Assert.fail("Expected ApiException for KM discover with SSRF-blocked URL");
        } catch (ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for KM discover with SSRF-blocked URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in KM discover response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — MCP proxy create with non-matching URL returns 400",
    ...
    public void testAllowModeEnabled_MCPProxyCreateWithBlockedURLRejected() throws Exception {
        try {
            ...
            proxyProps.setName("SSRFProxyTest");
            proxyProps.setContext("/ssrfproxytest");
            ...
            Assert.fail("Expected ApiException for MCP proxy create with SSRF-blocked URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for MCP proxy create with SSRF-blocked URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in MCP proxy create response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — API creation with blocked backend URL returns 400",
    ...
    public void testAllowModeEnabled_APICreateWithBlockedBackendURLRejected() throws Exception {
        try {
            ...
            body.setName("SSRFBackendBlockTest");
            body.setContext("/ssrfbackendblock");
            ...
            Assert.fail("Expected ApiException for API creation with SSRF-blocked backend URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for API creation with SSRF-blocked backend URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in API create response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — API update with blocked backend URL returns 400",
    ...
    public void testAllowModeEnabled_APIUpdateWithBlockedBackendURLRejected() throws Exception {
        try {
            ...
            Assert.fail("Expected ApiException for API update with SSRF-blocked backend URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for API update with SSRF-blocked backend URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in API update response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — create MCP server from existing API with blocked endpoint returns 400",
    ...
    public void testAllowModeEnabled_MCPServerCreateFromAPIWithBlockedEndpointRejected() throws Exception {
        try {
            ...
            mcpDTO.setName("SSRFMcpFromAPITest");
            mcpDTO.setContext("/ssrfmcpfromapitest");
            ...
            Assert.fail("Expected ApiException for MCP server create from API with SSRF-blocked endpoint");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for MCP server create from API with SSRF-blocked endpoint");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in MCP server create response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — MCP server update with blocked backend URL returns 400",
    ...
    public void testAllowModeEnabled_MCPServerUpdateWithBlockedEndpointRejected() throws Exception {
        ...
        try {
            ...
            Assert.fail("Expected ApiException for MCP server update with SSRF-blocked backend URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for MCP server update with SSRF-blocked backend URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in MCP server update response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — MCP server backend update with blocked URL returns 400",
    ...
    public void testAllowModeEnabled_MCPServerBackendUpdateWithBlockedURLRejected() throws Exception {
        ...
        try {
            ...
            Assert.fail("Expected ApiException for MCP server backend update with SSRF-blocked URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for MCP server backend update with SSRF-blocked URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in MCP server backend update response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — OpenAPI import with blocked endpoint in additionalProperties returns 400",
    ...
    public void testAllowModeEnabled_OpenAPIImportWithBlockedEndpointRejected() throws Exception {
        try {
            ...
            additionalProperties.put("name", "SSRFOASImportTest");
            additionalProperties.put("context", "/ssrfoasimporttest");
            ...
            Assert.fail("Expected ApiException for OpenAPI import with SSRF-blocked endpoint");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for OpenAPI import with SSRF-blocked endpoint");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in OpenAPI import response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — AsyncAPI import with blocked endpoint in additionalProperties returns 400",
    ...
    public void testAllowModeEnabled_AsyncAPIImportWithBlockedEndpointRejected() throws Exception {
        try {
            ...
            File asyncAPIFile = new File(getAMResourceLocation()
                    + File.separator + "ssrf" + File.separator + "ssrf-test-asyncapi.yaml");
            JSONObject endpointUrl = new JSONObject();
            endpointUrl.put("url", BLOCKED_URL);
            JSONObject endpointConfig = new JSONObject();
            ...
            endpointConfig.put("production_endpoints", endpointUrl);
            JSONObject additionalProperties = new JSONObject();
            additionalProperties.put("name", "SSRFAsyncAPIImportTest");
            additionalProperties.put("context", "/ssrfasyncimporttest");
            additionalProperties.put("version", "1.0.0");
            additionalProperties.put("provider", user.getUserName());
            additionalProperties.put("endpointConfig", endpointConfig);
            restAPIPublisher.importAsyncAPIDefinition(asyncAPIFile, additionalProperties.toJSONString());
            Assert.fail("Expected ApiException for AsyncAPI import with SSRF-blocked endpoint");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for AsyncAPI import with SSRF-blocked endpoint");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in AsyncAPI import response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — GraphQL schema import from blocked URL returns 400",
    ...
    public void testAllowModeEnabled_GraphQLImportByBlockedURLRejected() throws Exception {
        try {
            enableTenantAllowMode(new String[]{"*.allowed.example.com"});
            JSONObject additionalProperties = new JSONObject();
            additionalProperties.put("name", "SSRFGraphQLImportTest");
            additionalProperties.put("context", "/ssrfgqlimporttest");
            additionalProperties.put("version", "1.0.0");
            additionalProperties.put("provider", user.getUserName());
            restAPIPublisher.importGraphqlSchemaDefinitionByURL(
                    BLOCKED_URL, additionalProperties.toJSONString());
            Assert.fail("Expected ApiException for GraphQL import from SSRF-blocked URL");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for GraphQL schema import from SSRF-blocked URL");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in GraphQL import response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — MCP server from OpenAPI with blocked endpoint in additionalProperties returns 400",
    ...
    public void testAllowModeEnabled_MCPServerFromOpenAPIWithBlockedEndpointRejected() throws Exception {
        try {
            ...
            endpointConfig.put("production_endpoints", endpointUrl);
            JSONObject additionalProperties = new JSONObject();
            additionalProperties.put("name", "SSRFMcpFromOASTest");
            additionalProperties.put("context", "/ssrfmcpfromoastest");
            additionalProperties.put("version", "1.0.0");
            additionalProperties.put("provider", user.getUserName());
            additionalProperties.put("endpointConfig", endpointConfig);
            restAPIPublisher.createMCPServerFromOpenAPI(openAPIFile, additionalProperties.toJSONString());
            Assert.fail("Expected ApiException for MCP server from OpenAPI with SSRF-blocked endpoint");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for MCP server from OpenAPI with SSRF-blocked endpoint");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in MCP server from OpenAPI response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — API ZIP import with blocked endpoint URL returns 400",
    ...
    public void testAllowModeEnabled_APIZIPImportWithBlockedEndpointRejected() throws Exception {
        try {
            ...
            File apiZip = new File(getAMResourceLocation()
                    + File.separator + "ssrf" + File.separator + "ssrf-test-api-blocked.zip");
            buildImportExportApiClient().importAPIWithHttpInfo(apiZip, false, false, false);
            Assert.fail("Expected ApiException for API ZIP import with SSRF-blocked endpoint");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for API ZIP import with SSRF-blocked endpoint");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in API ZIP import response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: allow mode enabled — MCP server ZIP import with blocked endpoint URL returns 400",
    ...
    public void testAllowModeEnabled_MCPServerZIPImportWithBlockedEndpointRejected() throws Exception {
        try {
            ...
            File mcpZip = new File(getAMResourceLocation()
                    + File.separator + "ssrf" + File.separator + "ssrf-test-mcp-blocked.zip");
            buildImportExportApiClient().importMCPServerWithHttpInfo(mcpZip, false, false, false, false, false, null);
            Assert.fail("Expected ApiException for MCP server ZIP import with SSRF-blocked endpoint");
        } catch (org.wso2.am.integration.clients.publisher.api.ApiException e) {
            Assert.assertEquals(e.getCode(), HttpStatus.SC_BAD_REQUEST,
                    "Expected HTTP 400 for MCP server ZIP import with SSRF-blocked endpoint");
            Assert.assertTrue(e.getResponseBody() != null && e.getResponseBody().contains("not trusted"),
                    "Expected SSRF block error in MCP server ZIP import response body, got: " + e.getResponseBody());
        } finally {
            restoreOriginalTenantConfig();
        }
    }
    ...
    @Test(groups = {"wso2.am"},
            description = "SSRF: super-tenant allow mode must not affect a new tenant whose config is at default",
    ...
    public void testAllowModeIsolation_NewTenantUnaffectedBySuperTenantAllowMode() throws Exception {
        try {
            ...
            ApiEndpointValidationResponseDTO superDto =
                    restAPIPublisher.validateEndpointRaw(BLOCKED_URL, apiId);
            Assert.assertNotNull(superDto.getError(), "Expected SSRF block for super tenant");
